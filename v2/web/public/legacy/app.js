...
const embeddedMode = routeParams.get("embedded") === "1";

function canonicalViewPath(view) {
  const mapping = {
    dashboard: "/dashboard",
    pool: "/pool",
    funnel: "/funnel",
...
function parentPathname() {
  try {
    return (window.top?.location?.pathname || window.location.pathname || "").replace(/\/+$/, "") || "/";
  } catch {
...
function navigateParent(path) {
  ...
  if (parentPathname() === path) return false;
...
function reportRoute(reportId) {
  return `/reports/${reportId}-report`;
}

async function renderInitialRoute() {
  const reportId = Number(routeParams.get("report") || "0");
  if (reportId) {
    state.view = "reports";
    await openReport(reportId, { syncParent: false });
    return;
  }
  const companyId = Number(routeParams.get("company") || "0");
  if (companyId) {
    state.view = "pool";
    await openCompany(companyId, { syncParent: false });
    return;
  }
  renderView(routeParams.get("view") || "dashboard", { syncParent: false });
}
...
function renderView(view, options = {}) {
  ...
  if ((options.syncParent ?? true) && navigateParent(canonicalViewPath(nextView))) return;
  state.view = nextView;
  clearDocumentStatusPolling();
  document.querySelectorAll(".nav-button").forEach((button) => {
...
async function openCompany(companyId, options = {}) {
  if ((options.syncParent ?? true) && navigateParent(companyRoute(companyId))) return;
...
async function openReport(reportId, options = {}) {
  if ((options.syncParent ?? true) && navigateParent(reportRoute(reportId))) return;
